...
void printCFG();
void removeNullProduction(RList* node);

int main()
{
	loadCFGFromFile("file.txt");
	...
	removeNullProduction(first_rlist);
...
void abc(char s)
{
	...
	if (first_rlist != nullptr)
	{
		for (RList* list = last_rlist; list != nullptr; list = list->prev_rlist)
		{
			for (Rule* r = list->head; r != nullptr; r = r->next)
			{
				string x = "";
			...
			for (Rule* r = list->head; r != nullptr; r = r->next)
			{
				for (int i = 0; r->rule[i].s != '\0'; i++)
				{
					if (r->rule[i].s == '@')
					{
						abc(list->s.s);
						deleteRule(list,r);
						cout << "Removing Null rule from " << list->s.s<<"\n";
						printCFG();
						cout << endl;
